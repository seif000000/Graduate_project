...
import {
  ...
  ShieldCheck, LogOut, Settings, Bell, HelpCircle,
  UserCircle, Mail
} from 'lucide-react';
...
const LayoutUser = ({ children, title }) => {
  ...
  return (
    <div className="flex min-h-screen bg-[#FAF9F6]" dir="rtl">
      {/* Sidebar */}
      <aside className="w-72 bg-primary-950 h-screen fixed right-0 top-0 flex flex-col z-50 shadow-2xl">
        {/* Brand */}
        <div className="p-6 border-b border-white/10">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 bg-gradient-to-br from-primary-400 to-primary-600 rounded-xl flex items-center justify-center shadow-lg">
              🏥
            </div>
            <div>
              <h1 className="text-xl font-black text-white tracking-tight">مُسند</h1>
              <p className="text-[10px] text-primary-300 font-bold uppercase tracking-widest opacity-60">Musnad Platform</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-grow overflow-y-auto py-4 px-3 sidebar-scroll">
          {userNav.map(group => (
            <div key={group.section} className="mb-6">
              <p className="px-3 text-[10px] font-black text-primary-400 uppercase tracking-[0.2em] mb-2 opacity-50">{group.section}</p>
      ...
      <div className="flex-grow mr-72 flex flex-col min-h-screen">
        {/* Topbar */}
        <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-slate-200 px-8 py-4 flex items-center justify-between shadow-sm">
          <h2 className="text-lg font-black text-slate-800">{title || 'منصة مسند'}</h2>
          <div className="flex items-center gap-3">
             <Link to="/inbox" className="w-9 h-9 rounded-xl bg-slate-50 flex items-center justify-center hover:bg-slate-100 text-slate-500 transition-all relative">
              <Mail size={18} />
             </Link>
             <Link to="/notifications" className="w-9 h-9 rounded-xl bg-slate-50 flex items-center justify-center hover:bg-slate-100 text-slate-500 transition-all relative">
              <Bell size={18} />
             </Link>

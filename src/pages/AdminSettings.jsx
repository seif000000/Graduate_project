...
const AdminSettings = () => {
  ...
  const [notifs, setNotifs] = useState([
    { id: 'new_pharmacy', title: 'طلبات توثيق جديدة', desc: 'عند تسجيل صيدلية جديدة تحتاج للمراجعة', enabled: true },
    { id: 'reports', title: 'بلاغات المستخدمين', desc: 'عند وصول شكوى أو بلاغ جديد', enabled: true },
    { id: 'sos', title: 'طلبات استغاثة عاجلة', desc: 'عند نشر طلب استغاثة بأولوية عالية', enabled: false },
  ]);
    ...
    <div className="space-y-8 pb-12" dir="rtl">
      <header className="space-y-1 text-right">
        ...
        <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">
          تخصيص إعدادات لوحة الإدارة والنظام
        </p>
      </header>
        ...
        <div className="space-y-2">
          {tabs.map(tab => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-5 py-3.5 rounded-xl font-bold transition-all text-right text-sm ${
                activeTab === tab.id
        ...
        <div className="lg:col-span-3 bg-slate-900 border border-white/5 rounded-2xl p-8 space-y-8">
          {activeTab === 'general' && (
            <div className="space-y-8">
              ...
              <section className="space-y-4">
                <h3 className="text-base font-black text-white border-r-4 border-red-500 pr-3">معلومات الأدمن</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block">الاسم الكامل</label>
                    <input
                      type="text"
              ...
              {/* Language */}
              <section className="space-y-4">
                <h3 className="text-base font-black text-white border-r-4 border-red-500 pr-3">اللغة والمنطقة</h3>
                <div className="max-w-xs">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2">لغة النظام</label>
          ...
          {activeTab === 'notifications' && (
            <div className="space-y-6">
              <h3 className="text-base font-black text-white border-r-4 border-red-500 pr-3">تفضيلات الإشعارات</h3>
                ...
                {notifs.map((item) => (
                  <div key={item.id} className="flex items-center justify-between p-5 rounded-xl border border-white/5 bg-slate-800/50 hover:bg-slate-800 transition-all">
                    ...
                    <button
                      ...
                      className={`w-12 h-6 rounded-full transition-all duration-300 relative flex-shrink-0 ml-4 ${item.enabled ? 'bg-red-500' : 'bg-slate-700'}`}
                    >
                      <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-all duration-300 ${item.enabled ? 'right-1' : 'left-1'}`} />
                    </button>
                  </div>
                ))}
              </div>
          ...
          {activeTab === 'security' && (
            <div className="space-y-8">
              {/* Change Password */}
              <section className="space-y-4">
                <h3 className="text-base font-black text-white border-r-4 border-red-500 pr-3">تغيير كلمة المرور</h3>
                <div className="space-y-3 max-w-md">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 h-11 px-6 rounded-xl bg-red-500 hover:bg-red-600 text-white font-black text-sm shadow-[0_0_15px_rgba(239,68,68,0.3)] transition-all"
                  >
                    <Lock size={15} /> تحديث كلمة المرور
                  </button>
                </div>
              </section>
              ...
              {/* Danger Zone */}
              <section className="space-y-4">
                <h3 className="text-base font-black text-red-400">منطقة الخطر</h3>
                <div className="p-6 rounded-xl border-2 border-dashed border-red-500/20 bg-red-500/5 text-right space-y-3">
                  <h4 className="text-white font-black">تعطيل النظام</h4>
                  <p className="text-xs text-slate-400 font-medium leading-relaxed">
                    يمكن تعطيل النظام مؤقتاً لأغراض الصيانة. سيتم إخطار المستخدمين تلقائياً.
                  </p>
                  <button
                    onClick={() => toast.error('غير متاح في بيئة الإنتاج ⚠️')}
